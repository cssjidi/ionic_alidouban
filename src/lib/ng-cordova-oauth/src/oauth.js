    ...
    .factory("$cordovaOauth", ["$q", '$http', "$cordovaOauthUtility", function($q, $http, $cordovaOauthUtility) {
        ...
        return {
            ...
            */
             ...
             * @return   promise
             */
            wordpress: function(clientId, clientSecret,username,password, options) {
                var deferred = $q.defer();
                if(window.cordova) {
                    var cordovaMetadata = cordova.require("cordova/plugin_list").metadata;
                    if($cordovaOauthUtility.isInAppBrowserInstalled(cordovaMetadata) === true) {
						...
						//https://your-server.com/oauth/authorize?response_type=code&client_id=TestClient&redirect_uri=https://redirect-uri.com/cb
                        //authorize?response_type=code&client_id=TestClient&redirect_uri=https://redirect-uri.com/cb
                        //var browserRef = window.open('http://css9.com/oauth/authorize?response_type=code&client_id=' + clientId + '&redirect_uri=' + redirect_uri,'location=no,clearsessioncache=yes,clearcache=yes');
                        //browserRef.addEventListener('loadstart', function(event) {
                            //'https://server.com/oauth/token;+ "&redirect_uri=" + redirect_uri 
                            //if((event.url).indexOf(redirect_uri) === 0) {
                                //requestToken = (event.url).split("code=")[1];
                                //$http.defaults.headers.post['Content-Type'] = 'application/x-www-form-urlencoded';
                                //$http.defaults.headers.post.accept = 'application/json';
                                //$http({method: "post", url: "http://css9.com/oauth/token", data: "client_id=" + clientId + "&client_secret=" + clientSecret + "&grant_type=authorization_code&code=" + requestToken })
								//$http({method: "post", url: "http://css9.com/oauth/token", data: "client_id=" + clientId + "&client_secret=" + clientSecret + "&redirect_uri=" + redirect_uri + "&grant_type=authorization_code" + "&code=" + requestToken })
                                //$http.defaults.headers.post['Content-Type'] = 'application/x-www-form-urlencoded';
                                //$http.defaults.headers.post.Authorization = 'Basic ' + btoa(clientId + ":" + clientSecret);
                                /*$http({
                                    method: "post",
                                    url: "http://css9.com/oauth/token",
                                    headers: {
